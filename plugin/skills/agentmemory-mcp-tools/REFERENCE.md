...
| `memory_agent_event_prune` |  | `maxAgeDays`: number, `maxCount`: number, `batch`: number, `dryRun`: boolean | Prune the agent event log by age and/or count in bounded batches. Returns how many events were removed (or would be removed in dry-run mode) split by age and count reasons. |
...
| `memory_audit_chain` |  | `offset`: number, `limit`: number, `includeLinks`: boolean, `operation`: string, `functionId`: string, `dateFrom`: string, `dateTo`: string | Build the tamper-evident audit hash chain over recorded audit entries. Returns the recomputed head hash, the persisted head pointer, per-link hashes (optional), and any row integrity issues. |
| `memory_audit_chain_verify` |  | `offset`: number, `limit`: number, `includeLinks`: boolean, `operation`: string, `functionId`: string, `dateFrom`: string, `dateTo`: string, `expectedHeadHash`: string, `expectedCount`: number, `expectedFirstEntryId`: string, `expectedLastEntryId`: string, `allowUnanchored`: boolean | Verify the audit hash chain against caller-supplied or persisted anchors (expected head hash, count, first/last entry id, or a provided chain). Returns valid plus the exact mismatches and which integrity properties were checked. |
...
| `memory_ledger` |  | `project`: string, `state`: string, `type`: string, `lane`: string, `reviewState`: string, `includeSourceCards`: boolean, `limit`: number, `offset`: number | List memories as a Memory Ledger with type, scope, lifecycle, review state, source counts, last use, and optional source cards. |
...
| `memory_proposal_apply` |  | `teamId`: string, `proposalId`*: string, `reason`: string, `actorId`: string | Apply an approved proposal by invoking the underlying lifecycle function and recording the application in the audit trail. |
| `memory_proposal_approve` |  | `teamId`: string, `proposalId`*: string, `reason`: string, `actorId`: string | Approve a pending proposal. Requires the proposal's required permissions; self-approval is blocked unless the deployment opts in via AGENTMEMORY_ALLOW_SELF_APPROVAL. |
| `memory_proposal_create` |  | `teamId`: string, `project`: string, `action`*: string, `title`: string, `reason`: string, `change`*: object, `provenance`: object, `actorId`: string | Propose a memory lifecycle change (create/update/expire/archive/restore/delete) for team review instead of applying it directly. Requires project:write. |
| `memory_proposal_list` |  | `teamId`: string, `project`: string, `action`: string, `status`: string, `targetMemoryId`: string, `limit`: number, `offset`: number, `actorId`: string | List team memory proposals with optional project/action/status/target filters and pagination. Requires project:read. |
| `memory_proposal_reject` |  | `teamId`: string, `proposalId`*: string, `reason`: string, `actorId`: string | Reject a pending proposal. Requires the same governance the proposal needs so a non-governance actor cannot veto a governance-gated delete. |
...
| `memory_sync_peer_set_status` |  | `peerId`*: string, `enabled`: boolean | Enable or disable a registered sync peer. Recomputes peer health from config alone, clearing run-derived block reasons so a peer an untrusted run marked blocked can recover. |
