...
async function main() {
	...
	fetch(`${REST_URL}/agentmemory/observe`, {
		...
		body: JSON.stringify({
			...
			data: {
				tool_name: toolName,
				tool_input: safeMetadata(typeof toolInput === "string" ? toolInput.slice(0, 4e3) : JSON.stringify(safeMetadata(toolInput) ?? "").slice(0, 4e3)),
				error: typeof error === "string" ? safeMetadata(error.slice(0, 4e3)) : JSON.stringify(safeMetadata(error) ?? "").slice(0, 4e3),
				lineage
			}
		}),
