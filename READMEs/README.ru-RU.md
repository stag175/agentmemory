...
- `@agentmemory/mcp` как MCP-сервер (проксирует все 74 инструмент, когда `AGENTMEMORY_URL` указывает на работающий сервер agentmemory; локально откатывается к 7 инструментам, если сервер недоступен)
