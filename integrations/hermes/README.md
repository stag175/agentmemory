...
an MCP server with all 74 memory tools:
...
This gives Hermes access to all 74 MCP tools and enables the agentmemory memory provider. Start the server separately:
...
Session lifecycle and capture posts are stamped `captureSource: "automatic_hook"`, so they respect the server-side capture kill switches (`AGENTMEMORY_CAPTURE_PAUSED`, incognito mode) exactly like agentmemory's native hooks: pause capture and Hermes session/tool posts are skipped too.
