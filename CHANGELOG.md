...
- **`POST /agentmemory/graph/dsl` — multi-hop knowledge graph query DSL** (Q3 2026 roadmap). A small Cypher-inspired language for multi-hop questions over the knowledge graph: one `MATCH` pattern chain with typed nodes/edges, directed/undirected edges, and variable-length hops (`*min..max`, capped at 5); a `WHERE` conjunction over node names/types/properties and edge weights (`=`, `!=`, `~` case-insensitive substring, numeric comparisons); `RETURN paths | nodes | edges | <vars>`; and `LIMIT`. Re-using a node variable joins on the same node, which expresses cycles; matched paths are otherwise simple (no repeated node or edge). Parser and evaluator are pure and dependency-free (`src/functions/graph-dsl.ts`); no `eval`, no user-input regexes, work bounded by a visit budget. The `mem::graph-dsl` handler reuses the #814 machinery — live enumeration raced against the wall-clock budget with the top-degree snapshot as a degraded fallback (`warning` field, mirroring `/graph/query`). Parse errors return HTTP 400 with the failing character offset. Gated by `GRAPH_EXTRACTION_ENABLED`.
- **Hermes per-tool-call capture** (Q3 2026 roadmap: OpenClaw-surface parity). The Hermes memory provider gains an `on_post_tool_use(tool_name, tool_input, tool_output)` hook that posts each individual tool invocation to `/agentmemory/observe` in the background (`hookType: "post_tool_use"`), distinct from `sync_turn`'s turn-level "conversation" observation. Long string fields truncate at 2000/8000 chars with a marker; structured values pass through for server-side sanitization. `session/start` / `session/end` payloads now carry the same `captureSource`/`hookType` telemetry stamps as the native hooks. Plugin surface is now 7 hooks (plugin.yaml 0.9.0).
- **OpenSSF Scorecard enrollment + workflow hardening** (Q3 2026 roadmap). New `scorecard.yml` workflow (weekly + on push to main, publishes results + SARIF to code scanning) and `dependency-review.yml` (fails PRs introducing high-severity-vulnerable dependencies). All GitHub Actions across workflows pinned to full commit SHAs with version comments (Dependabot keeps them bumped); `ci.yml` gains a least-privilege top-level `permissions: contents: read`. OpenSSF Scorecard badge added to the README.
